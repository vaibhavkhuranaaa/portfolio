...
Generated from the approved portfolio catalog at `2026-08-01T00:00:00.000Z`.
...
| [Fintech Customer Retention Command Center](https://saas-revenue-retention-command-center.vaibhavkhurana.workers.dev/fintech) | A deployed, read-only retention ledger over a minimized COFINFAD V1 customer-health projection. | Live claim; verified public dashboard/API | Public verification returned HTTP 200 for the dashboard and filtered API; the API exposes exactly six derived fields and excludes raw financial records, identifiers, timestamps, free text, and granular demographics. | `3265d42` |
| [LLM Gateway with Rate Limiting, Fallback Routing, and Observability](https://llm-gateway-demo-lfpwcuk37a-uc.a.run.app) | A public synthetic-only gateway control plane that demonstrates tenant policy, rate and cost controls, deterministic fallback, and redacted audit receipts. | Live claim; verified public dashboard/API | The public dashboard, event stream, and fixed fallback scenario returned HTTP 200; arbitrary prompt submissions remain refused with HTTP 403. The service uses deterministic provider test doubles and retains no prompt text. | `81778ecb2ee0af827be2948e6071d9f3a097bc45` |
